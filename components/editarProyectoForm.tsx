...
const EditarProyectoForm = ({isOpen, onClose, editarDatos, cliente}) => {
    
    const [recursos,setRecursos] = useState([])

    const [nombre, setNombre] = useState(cliente['Nombre']);
    const [descripcion, setDescripcion] = useState(cliente['Descripcion']);
    const [lider, setLider] = useState(cliente['Lider']);
    const [estado, setEstado] = useState(cliente['Estado']);
    const [fechaIni, setFechaIni] = useState(cliente['FechaIni']);
    const [fechaFin, setFechaFin] = useState(cliente['FechaFin']);
    ...
    <div className='modal-container' style={{ display: isOpen ? 'grid': 'none'}}> 
      <div className='modal-body'>
        ...
        <div class="input-group mb-3">
            <span class="input-group-text" id="inputGroup-sizing-default">Id del Proyecto</span>
            <span class="input-group-text" id="inputGroup-sizing-default">{cliente['id']}</span>
        </div>
        ...
        <button className='inline-block rounded border-2 border-primary px-6 pb-[6px] pt-2 text-xs font-medium uppercase leading-normal text-primary transition duration-150 ease-in-out hover:border-primary-600 hover:bg-neutral-500 hover:bg-opacity-10 hover:text-primary-600 focus:border-primary-600 focus:text-primary-600 focus:outline-none focus:ring-0 active:border-primary-700 active:text-primary-700 dark:hover:bg-neutral-100 dark:hover:bg-opacity-10" data-te-ripple-init' onClick={()=> {
            editarDatos({Nombre: nombre, Descripcion: descripcion,Lider: lider,Estado: estado, FechaIni: fechaIni,FechaFin: fechaFin,Id: id});
            onClose()}}>
                Guardar
        </button>   
